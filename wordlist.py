...
df_transposed = pd.read_csv('transposed_miscarriage5.csv', index_col=0)

# Sort the dataframe by the 'miscarriage' column in descending order
df_sorted = df_transposed.sort_values(by='miscarriage', ascending=False)

# Convert the dataframe to a dictionary with words and their frequencies
data = df_sorted['miscarriage'].to_dict()
...
truncated_words = list(limited_data.keys())
truncated_words_str = ','.join(truncated_words)
print("Truncated list of words (up to 200 characters):")
print(truncated_words_str)
...
def generate_better_wordcloud(data, title, mask=None, color_func=None):
    ...
    image.save('/Users/matildeelene/Desktop/Applied/traumacloud/wordclouds/text_only_miscarriage5.png', "PNG")

# Call the function to generate and display the word cloud with limited text length
generate_better_wordcloud(limited_data, "miscarriage5",mask=cloud_mask, color_func=off_white_color_func)
